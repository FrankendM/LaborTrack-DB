...
if ($method === 'GET') {
    ...
    json_ok(array_map(fn($r) => [
        ...
        'requires_statutory_deductions' => (bool)$r['requires_statutory_deductions'],
    ], $rows));
}
...
if ($method === 'POST') {
    ...
    if ($name === '') json_err('type_name is required.');
    $pdo = getDB();
    $pdo->prepare(
        'INSERT INTO employment_types (type_name, requires_statutory_deductions) VALUES (?, ?)'
    )->execute([
        $name,
        isset($body['requires_statutory_deductions']) ? (int)(bool)$body['requires_statutory_deductions'] : 1,
    ]);
...
if ($method === 'PUT') {
    ...
    $name = str($body, 'type_name');
    if (!$id)       json_err('employment_type_id is required.');
    if ($name === '') json_err('type_name is required.');
    $stmt = getDB()->prepare(
        'UPDATE employment_types SET type_name = ?, requires_statutory_deductions = ? WHERE employment_type_id = ?'
    );
    $stmt->execute([
        $name,
        isset($body['requires_statutory_deductions']) ? (int)(bool)$body['requires_statutory_deductions'] : 1,
    ...
    if ($stmt->rowCount() === 0) json_err('Employment type not found.', 404);
